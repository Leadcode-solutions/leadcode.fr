import React from 'react'
import { MagnifyingGlassIcon } from '@heroicons/react/20/solid'
import {ItemType} from '../types'
import {Link, usePage} from "@inertiajs/inertia-react";
import { classNames } from '../helpers'

type Props = {
  navigation: ItemType[]
}

const SearchBar = () => {
  return (
    <div className="mt-5 px-3">
      <label htmlFor="search" className="sr-only">
        Search
      </label>
      <div className="relative mt-1 rounded-md shadow-sm">
        <div
          className="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3"
          aria-hidden="true"
        >
          <MagnifyingGlassIcon className="mr-3 h-4 w-4 text-gray-400" aria-hidden="true" />
        </div>
        <input
          type="text"
          name="search"
          id="search"
          className="block w-full rounded-md border-gray-300 pl-9 focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
          placeholder="Search"
        />
      </div>
    </div>
  )
}

export default ({ navigation }: Props) => {
  const page = usePage()
  const list = page.component.split('/')
  const component = list[1]

  return (
    <>
      <div className="hidden lg:sticky lg:top-0 lg:flex w-48 h-screen border-r border-theme">
        <div className="flex flex-col ">
          <SearchBar />
          <div className="flex flex-col p-4">
            { navigation.map((item) => (
              <Link
                key={item.name}
                href={item.href}
                className={classNames(
                  'px-2 py-1 rounded-md',
                  component.endsWith(item.component) ? 'bg-theme-active' : 'text-theme text-opacity-50'
                )}
              >
                {item.name}
              </Link>
            ))}
          </div>
        </div>

      </div>
    </>
  )
}
